...
document.observe("dom:loaded", PCNL_START_FUNCTION = function()
{
	...
	var PostcodeNl_Api = {
		...
		lookupPostcode: function (prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, event)
		{
			...
			if(prefix == 'billing:' && $('billing-allshipping') && $('billing-allshipping').getValue() > 0) {
				
				var billingAddressId = jQuery('#billing-allshipping').val();
				
				//make it read only
				if($(prefix+ "postcode")) {
					$(prefix+ "postcode").setAttribute("readOnly","true");
				...
				if($(prefix+ "postcode_input")) {
					$(prefix+ "postcode_input").setAttribute("readOnly","true");
				...
				if($(prefix+ "postcode_inputdegit")) {
					$(prefix+ "postcode_inputdegit").setAttribute("readOnly","true");
			...
			//house number addtion field settings
		    var housenumber_mixed = addressJson[billingAddressId].street2;
			...
			var housenumber_addition_select = null;
			var housenumber = housenumber_match ? housenumber_match[1].trim() : housenumber_mixed;

			if($(prefix+ "housenumber") && $(prefix+ "housenumber").getValue() == '') {
                $(prefix+ "housenumber").removeAttribute("readOnly");
				$(prefix+ "housenumber").setStyle({
						backgroundColor: 'white'
				});
				
            } else if($(prefix +'postcode_housenumber') && housenumber != '') {
				$(prefix+ "postcode_housenumber").setAttribute("readOnly","true");
				$(prefix +'postcode_housenumber').setValue(housenumber);
				$(prefix+ "postcode_housenumber").setStyle({
									backgroundColor: '#f2f2f2',
							});
			}else if(housenumber != '') {
				
				$(prefix +'housenumber').setValue(housenumber);
                $(prefix+ "housenumber").setAttribute("readOnly","true");
            }
			...
			if (housenumber_addition != '') {
				
				this.createPostcodeHouseNumberAddition(prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4,[housenumber_addition]);
					
				 $(prefix +'postcode_housenumber_addition').setValue(housenumber_addition);
				 $(prefix +'postcode_housenumber_addition').setAttribute("disabled","disabled");
				 $('billing-addition-house').setStyle({
						backgroundColor: '#f2f2f2'
					});
					...
					if($(prefix + 'housenumber-addition')) {
							$(prefix + 'housenumber-addition').remove();
					}
			}
				...
				/* housenumber_addition = addressJson[billingAddressId].houseNumberAddition;
				housenumber_addition_select
				 */
				/* pcnlapi.updatePostcodeLookup(addressJson[billingAddressId], housenumber_addition, housenumber_addition_select, prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, event);  */
				...
				//remove read only
				if($(prefix+ "postcode")) {
					$(prefix+ "postcode").removeAttribute("readOnly");
					$(prefix+ "postcode").setStyle({
						backgroundColor: '#ffffff'
					});
				}
				...
				if($(prefix+ "postcode_input")) {
					$(prefix+ "postcode_input").removeAttribute("readOnly");
					$(prefix+ "postcode_input").setStyle({
						backgroundColor: '#ffffff'
				});
				}
				if($(prefix+ "postcode_inputdegit")) {
					$(prefix+ "postcode_inputdegit").removeAttribute("readOnly");
					$(prefix+ "postcode_inputdegit").setStyle({
						backgroundColor: '#ffffff'
					});
				}
				...
				if($(prefix+ "street1")) {
                $(prefix+ "street1").setStyle({
                    backgroundColor: '#ffffff'
                });
				}
            if($(prefix+ "city")) {
                $(prefix+ "city").setStyle({
                    backgroundColor: '#ffffff'
                });
            }
			...
			var postcode = $(prefix + 'postcode_input').getValue()+ $(prefix + 'postcode_inputdegit').getValue();
			...
			// added  by parth to check pincode length
			...
			if (this.requestCache[url] === undefined)
			{
				...
				new Ajax.Request(url,
				{
					...
					onComplete: function(transport)
					{
						...
						//console.log('final data : ' + json.message);
						var errmsg = json.message;
						//console.log("main message 1" + errmsg);
						/*
						if(errmsg == "Onbekende postcode + huisnummer combinatie.") {
							//alert('u r in if');
							$('submitbtn').hide();
						}else{
							$('submitbtn').show();
			...
			{
				//console.log('else data : ' + this.requestCache[url].message);
				var errmsg = this.requestCache[url].message;
				//console.log("main message 2" + errmsg);
				if(errmsg == "Onbekende postcode + huisnummer combinatie.") {
					//alert('u r in if');
					$('submitbtn').hide();
				}else{
		...
		updatePostcodeLookup: function(data, housenumber_addition, housenumber_addition_select, prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, event)
		{
			...
			if (data.postcode !== undefined)
			{
				...
				if (postcodeChange)
					pcnlFireEvent($(prefix + postcodeFieldId), 'change');
                                ...
                                var postcodedegit=data.postcode.slice(-2);

				$(prefix + 'postcode_input').setValue(data.postcode.substr(0,4));
                                $(prefix + 'postcode_inputdegit').setValue(postcodedegit);
				...
				if (data.houseNumberAddition == null && (housenumber_addition_select == housenumber_addition || (housenumber_addition_select == '__none__' && housenumber_addition == '')))
				{
					...
					{
						console.log("u r in validation passed");
				...
				{ 
					...
					additionSelect.setValue(data.houseNumberAddition);
                                        /* Added By Helios start */
                                        $(prefix +'postcode_housenumber_addition').addClassName('validation-failed'); 
					hasAdvice = true;
                                        /* Added By Helios End */
					// $(prefix + 'postcode_housenumber_addition').addClassName('validation-passed');  // Comented by helios 
				}
				else
				{
					// Address has only one valid addition, and it is the 'no addition' option
				...
				if (!hasAdvice)
				{
					console.log("hasAdvice validation passed");
			...
			else if (data.message !== undefined)
			{
				...
				//console.log("advice" + newAdvice.innerHTML);
				
				//console.log("prefix" + prefix);
				//console.log("target" + target);
				
				//////console.log("pretarget" + $(prefix + target).innerHTML);
		...
		toggleCountryPostcode: function (prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4)
		{
			...
			{  
				...
				if (!$(prefix +'postcode_input:wrapper'))
				{
					...
					{  
						...
						/*if (!$(prefix +'postcode_input:info'))
						{
							$(prefix + street1).up('li').insert({before: '<li id="' + prefix + 'postcode_input:info" class="wide pcnl-info"><div><label class="pcnl-info-label">'+ PCNLAPI_CONFIG.translations.infoLabel +'</label><div class="pcnl-info-text" id="' + prefix + 'postcode_input:info-text">'+ PCNLAPI_CONFIG.translations.infoText +'</div></div></li>'});
                                                ...
                                                /*if($(prefix +'country_id').getValue()!='NL')
                                                {
                                                    $(prefix + street1).up('li').insert({before: '<li id="' + prefix + 'postcode_input:wrapper" class="fields"><div class="field input-postcode"><label for="' + prefix + 'postcode_input" class="required">'+ PCNLAPI_CONFIG.translations.postcodeInputLabel +'<em class="required">*</em></label><div class="input-box"><div class="field-wrapper"><input type="text" title="'+ PCNLAPI_CONFIG.translations.postcodeInputTitle +'" id="' + prefix + 'postcode_input" value="" class="input-text required-entry" /></div></div></div><div class="field input-postcode pcnl-input-housenumber"><label for="' + prefix + 'postcode_housenumber" class="required">'+ PCNLAPI_CONFIG.translations.houseNumberLabel +' <em class="required">*</em></label><div class="input-box"><div class="field-wrapper"><input type="text" title="'+ PCNLAPI_CONFIG.translations.houseNumberTitle +'" name="' + prefix + 'postcode_housenumber" id="' + prefix + 'postcode_housenumber" value="" class="input-text pcnl-input-text-half required-entry" /></div></div></div></li>'});
                                                }
                                                else*/{ 
                                                    $(prefix + street1).up('li').insert({before: '<li id="' + prefix + 'postcode_input:wrapper" class="fields"><div class="field input-postcode"><label for="' + prefix + 'postcode_input" class="required">'+ PCNLAPI_CONFIG.translations.postcodeInputLabel +'<em class="required">*</em></label><div class="input-box"><div class="field-wrapper"><input type="text" title="'+ PCNLAPI_CONFIG.translations.postcodeInputTitle +'" id="' + prefix + 'postcode_input" name="' + prefix + 'postcode_input" value=""  maxlength="4" class="input-text required-entry postcode-input" placeholder="1234" autocomplete="1234"/><input type="text" title="'+ PCNLAPI_CONFIG.translations.postcodeInputTitle +'" id="' + prefix + 'postcode_inputdegit" name="' + prefix + 'postcode_inputdegit" value="" class="input-text required-entry" placeholder="AB" autocomplete="AB"/></div></div></div><div class="field input-postcode pcnl-input-housenumber"><label for="' + prefix + 'postcode_housenumber" class="required">'+ PCNLAPI_CONFIG.translations.houseNumberLabel +' <em class="required">*</em></label><div class="input-box"><div class="field-wrapper"><input type="text" title="'+ PCNLAPI_CONFIG.translations.houseNumberTitle +'" name="' + prefix + 'postcode_housenumber" id="' + prefix + 'postcode_housenumber" value="" class="input-text pcnl-input-text-half required-entry validate-custom-housenumber housenumber-input" placeholder="10" autocomplete="10"/></div></div></div></li>'});
                                                }
						//$(prefix + postcodeFieldId).up('li').insert({before: '<li id="' + prefix + 'postcode_input:wrapper" class="fields"><div class="field input-postcode"><label for="' + prefix + 'postcode_input" class="required">'+ PCNLAPI_CONFIG.translations.postcodeInputLabel +'<em class="required">*</em></label><div class="input-box"><div class="field-wrapper"><input type="text" title="'+ PCNLAPI_CONFIG.translations.postcodeInputTitle +'" id="' + prefix + 'postcode_input" value="" class="input-text required-entry postcode-input" placeholder="1234AB" /><input type="text" title="'+ PCNLAPI_CONFIG.translations.houseNumberTitle +'" name="' + prefix + 'postcode_housenumber" id="' + prefix + 'postcode_housenumber" value="" class="input-text pcnl-input-text-half required-entry housenumber-input" placeholder="1A" /></div></div></div></li>'});
						if (!$(prefix +'postcode_input:checkbox'))
						{
							$(prefix + postcodeFieldId).up('li').insert({before: '<li id="' + prefix + 'postcode_input:checkbox" class="wide control pcnl-manual-checkbox"><div class="fields"><div class="input-box"><input type="checkbox" title="'+ PCNLAPI_CONFIG.translations.postcodeInputTitle +'" id="' + prefix + 'postcode_input_checkbox" value="" class="checkbox " /><label for="' + prefix + 'postcode_input_checkbox">'+ PCNLAPI_CONFIG.translations.manualInputText +'</label></div></div></li>'});

							$(prefix + 'postcode_input_checkbox').observe('click', function () { pcnlapi.toggleCountryPostcode(prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4); });
						}
						$(prefix + postcodeFieldId).up('li').insert({before: '<li id="' + prefix + 'postcode_input:output" class="wide pcnl-hidden-field postcodenl-output"><div><label>'+ PCNLAPI_CONFIG.translations.outputLabel +'</label><div id="' + prefix + 'postcode_output" class="pcnl-address-text"></div></div></li>'});
					}
					...
					if (PCNLAPI_CONFIG.debug && window.console) {
                                        ...
                                        {        
                                            ...
                                            $(prefix +'postcode_input').observe('keyup', function(e) { 
											
											pcnlapi.checkpostcodevalidate(prefix);

											});
											+
											 /* Added By RP */
											//if someone try to edit the degit in postcode and degit is not valid then do edit again
											$(prefix +'postcode_inputdegit').observe('focus', function(e) { 
											
												if($(prefix +'postcode_input').getValue().length != 4) {
													document.getElementById(prefix +'postcode_input').focus();
												}
                                            ...
                                            $(prefix +'postcode_inputdegit').observe('change', function(e) { 
											
											    /* Added By RP */
												if($(prefix +'postcode_housenumber') && $(prefix +'postcode_housenumber').getValue() != '' && $(prefix +'postcode_inputdegit').getValue().length >= 2) {
													
													pcnlapi.lookupPostcode(prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, e); 
													
												} else if($(prefix +'postcode_inputdegit').getValue().length >= 2) {
													pcnlapi.lookupPostcode(prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, e); 
												}
												/* Added By RP */
					...
					$(prefix +'postcode_housenumber').observe('change', function(e) {
						...
						/* Added By RP */
						if($(prefix + 'housenumber')) {
							$(prefix + 'housenumber').setValue($(prefix +'postcode_housenumber').getValue());
						}
						/* Added By RP */
				...
				{ 
					this.showFields([prefix +'postcode_input',prefix + 'postcode_inputdegit', prefix +'postcode_housenumber', prefix +'postcode_housenumber_addition', prefix + 'postcode_input:info-text', prefix + 'postcode_input_checkbox']);
				}
				...
				{ 
					//$(prefix +'postcode_input').setValue($(prefix + postcodeFieldId).getValue());
                                        $(prefix +'postcode_input').setValue($(prefix + postcodeFieldId).getValue().substr(0,4));
                                        $(prefix +'postcode_inputdegit').setValue($(prefix + postcodeFieldId).getValue().slice(-2));
			...
			else
			{
				...
				if ($(prefix +'postcode_input_checkbox'))
				{
					...
					if(prefix == 'billing:' && $('billing-allshipping') && $('billing-allshipping').getValue() > 0) {
							...
							this.setFieldsReadonly([
								...
								//remove read only field for the housenumber
								if($(prefix +'housenumber').getValue() == '') {
									this.setFieldsReadonly([							
										prefix +'housenumber',
								], false);
					...
					this.setFieldsReadonly([prefix +'postcode_input',prefix +'postcode_inputdegit', prefix +'postcode_housenumber', prefix +'postcode_housenumber_addition'], true);
					this.hideFields([
					    prefix +'postcode_input',
                        prefix +'postcode_inputdegit',
                ...
                checkpostcodevalidate:function(prefix){
                    //console.log( $(prefix + 'postcode_input').getValue().length + "=="+$(prefix + 'postcode_input').getValue());
                    if ($(prefix + 'postcode_input').getValue().length==4) {
						if(prefix == 'billing:' && $('billing-allshipping') && $('billing-allshipping').getValue() > 0) {
							
						} else {
							$(prefix + 'postcode_inputdegit').value='';
							$(prefix + 'postcode_inputdegit').focus();
						}
		...
		toggleAddressFields: function(prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, values)
		{
			...
			{ 
				...
				this.setFieldsReadonly([prefix +'postcode_input',prefix +'postcode_inputdegit', prefix + 'postcode_housenumber', prefix + 'postcode_housenumber_addition'], false);
		...
		createPostcodeHouseNumberAddition: function (prefix, postcodeFieldId, countryFieldId, street1, street2, street3, street4, values, custom)
		{
			...
			else
			{
				// We're probably in the frontend
                                if(jQuery('#co-billing-form div').hasClass(prefix+'housenumber-addition')) 
                                {  
                                  //$(prefix.replace(":","-")+'addition-house').insert({after:'<select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select>'});
                                  $(prefix.replace(":","-")+'addition-house').update('<select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select>');
                                }
                                else if(jQuery('#form-validate div').hasClass(prefix+'housenumber-addition')) 
                                {  
                                  //$(prefix.replace(":","-")+'addition-house').insert({after:'<select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select>'});
                                  $(prefix.replace(":","-")+'addition-house').update('<select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select>');
                                }
                                   ...
                                   // $(prefix.replace(":","-")+'addition-house').update('<select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select>');
                                    $(prefix + 'postcode_housenumber').insert({after:'<div class="'+prefix +'housenumber-addition '+prefix.replace(":","-")+'housenumber-addition"  id="'+prefix +'housenumber-addition"><span class="extrtext">Toevoeging</span><div id="'+prefix.replace(":","-")+'addition-house"><select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select></div></div>'});
                                    //
                                    //
                                    //$(prefix + 'postcode_housenumber').insert({after:'<div class="'+prefix +'housenumber-addition '+prefix.replace(":","-")+'housenumber-addition  id="'+prefix +'housenumber-addition"><span class="extrtext">'+ Translator.translate('additional')+'</span><div id="'+prefix.replace(":","-")+'addition-house"><select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select></div></div>'});
                                    /* last added///$(prefix + 'postcode_housenumber').insert({after:'<div class="'+prefix +'housenumber-addition '+prefix.replace(":","-")+'housenumber-addition"  id="'+prefix +'housenumber-addition"><span class="inaddition">'+ Translator.translate('additional')+'</span><div id="'+prefix.replace(":","-")+'addition-house"><select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select></div></div>'});
                                     * 
                                     */
                                    //$(prefix + 'postcode_housenumber').insert({after: '<select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half additional-address">'+ options +'</select>'});
				
                                }
			 	//$(prefix + 'postcode_housenumber').insert({after:'<div class="housenumber-addition"><span class="inaddition">'+ Translator.translate('additional')+'</span><div id="addition-house"><select title="'+ PCNLAPI_CONFIG.translations.houseNumberAdditionTitle +'" name="'+ prefix + 'postcode_housenumber_addition" id="' + prefix + 'postcode_housenumber_addition" class="validate-select pcnl-input-text-half">'+ options +'</select></div><span class="addition-example">('+ Translator.translate('For example 2hg')+')</span></div>'});
		...
		{ 
			...
			{ 
				...
				if ($('country').getValue() == 'NL'){ //alert('change');
