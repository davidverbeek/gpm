...
if(isset(($_POST['hdn_filters']))) {
  switch($_POST['hdn_filters']) {
    ...
    case "6":
      $extra_where = "pmd.buying_price > (CASE WHEN (mcpet_af.value = 0) THEN CAST((mcped.value * mcpet.value) AS DECIMAL (10 , ".$scale.")) ELSE CAST((mcped.value) AS DECIMAL (10 , ".$scale.")) END)";
    break;
    ...
    case "8":
      $extra_where = "pmd.selling_price > (CASE WHEN (mcpet_af.value = 0) THEN CAST((mcped_selling_price.value * mcpet.value) AS DECIMAL (10 , ".$scale." )) ELSE CAST((mcped_selling_price.value) AS DECIMAL (10 , ".$scale." )) END)";
    break;
...
$columns = array(
  ...
  array( 'db' => '(CASE WHEN (mcpet_af.value = 0) THEN CAST((mcped.value * mcpet.value) AS DECIMAL (10 , '.$scale.' )) ELSE CAST((mcped.value) AS DECIMAL (10 , '.$scale.' )) END) AS gyzs_buying_price',  'dt' => $column_index["gyzs_buying_price"]),
  array( 'db' => '(CASE WHEN (mcpet_af.value = 0) THEN CAST((mcped_selling_price.value * mcpet.value) AS DECIMAL (10 , '.$scale.' )) ELSE CAST((mcped_selling_price.value) AS DECIMAL (10 , '.$scale.' )) END) AS gyzs_selling_price',  'dt' => $column_index["gyzs_selling_price"]),
