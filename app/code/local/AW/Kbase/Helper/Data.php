...
class AW_Kbase_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public static function parseQuery($query)
    {
        ...
        $q1 = 0;
        $q2 = 0;
        while(false !== $q1 = strpos($query, '"', $q2))
        {
            $phrases = array_merge($phrases, self::preparePhrase(substr($query, $q2, $q1 - $q2)));
            $q1++;
            if(false === $q2 = strpos($query, '"', $q1))
            {
                $phrases = array_merge($phrases, self::preparePhrase(substr($query, $q1)));
                break;
            }
            if($quoted = substr($query, $q1, $q2 - $q1))
                $phrases[] = $quoted;
            $q2++;
        }
        if( false !== $q2
        && $q2 < strlen($query)
        )   $phrases = array_merge($phrases, self::preparePhrase(substr($query, $q2)));
