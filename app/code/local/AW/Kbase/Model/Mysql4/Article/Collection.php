...
class AW_Kbase_Model_Mysql4_Article_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    protected $_sortingFields = array(
        AW_Kbase_Model_Source_Sorting::BY_POSITION => 'sort_order',
    ...
    protected function _joinCategoryStoreTable()
    {
        if(!array_key_exists(self::TABLE_ALIAS_STORE, $this->getSelect()->getPart(Zend_Db_Select::FROM)))
            $this->_joinCategoryTable(false)
                ->getSelect()
                    ->joinLeft(array(self::TABLE_ALIAS_CATEGORY_STORE => $this->getTable('kbase/category_store')),
                            self::TABLE_ALIAS_CATEGORY_ARTICLE.'.category_id='.self::TABLE_ALIAS_CATEGORY_STORE.'.category_id',
                            ''
                        );
...
//            if(Mage::app()->isSingleStoreMode()) return $this;
    ...
    public function addSearchFilter($words)
    {
        ...
        foreach($words as $word)
            $this
                ->getSelect()
                    ->where( 'main_table.article_title LIKE \'%'.mysqli_real_escape_string($word).'%\' OR '
                            // TECHTWO: enable searching through tags:
                            .'main_table.article_text LIKE \'%'.mysqli_real_escape_string($word).'%\' OR '
                            .'main_table.article_id IN (SELECT tag_article_id FROM '.$this->getTable('kbase/tag').' WHERE tag_name LIKE \'%'.mysqli_real_escape_string($word).'%\')');
    ...
    public function addAuthorFilter($author)
    {
        ...
        foreach($words as $word)
            if($word = trim($word))
            {
                ...
                $first = false;
                $condition .= 
                    self::TABLE_ALIAS_USER.'_f.firstname LIKE \'%'.mysqli_real_escape_string($word).'%\''
                    .' OR '
                    .self::TABLE_ALIAS_USER.'_f.lastname LIKE \'%'.mysqli_real_escape_string($word).'%\'';
            }
