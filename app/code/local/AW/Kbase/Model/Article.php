...
//    {
//        echo '<pre>', print_r($this->debug()), '</pre>';
//        die();
//        parent::save();
//    }
    ...
    {
        if(is_null($limit))
            $limit = self::getShortDescriptionLimit();

        $sd = strip_tags($this->getArticleText());

        if( !$limit
        ||  $limit >= strlen($sd)
        )   return $sd;
        else if (false === $pos = strpos($sd, ' ', $limit-1))
            return substr($sd, 0, $limit);
        else
            return substr($sd, 0, $pos);
    }
