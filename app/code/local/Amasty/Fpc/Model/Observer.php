...
HTML;
    ...
    public function defer_js_css($page) {
        
        //$mip = strpos($_SERVER['REQUEST_URI'], "multistepcheckout/index/product/");
        
        //if($mip != 1) { // Exclude checkout second step
            $css = '';
			$without_style = '';
            preg_match_all('/<link.*?>/', $page, $matches_css);
            foreach ($matches_css[0] as $value_css) {
                if(strpos($value_css,"styles-ie.css")>0) {    
                    if($ie_version == "ie8") {
                        $ie_8_css = "<!--[if lt IE 8]>\r\n".$value_css."\r\n<![endif]-->";
                    }
                    $value_css = $ie_8_css;
                }
                if(strpos($value_css,'rel="stylesheet"') > 0) {
                    $css .= "\r\n".$value_css;
                } else {
                    $without_style .= "\r\n".$value_css;
                }
                if(strpos($value_css,'sli-rac.css') < 0) {
                    $css .= "\r\n".$value_css;
                } 
            }
            $page = preg_replace('/<link.*?>/', '', $page);
            $page = preg_replace('#</body>#',$css.'</body>',$page);
            $page = preg_replace('#</head>#',$without_style.'</head>',$page);
       // }
        ...
        foreach ($matches[0] as $value) {
            if(strpos($value,"ds-sleight.js")>0 || strpos($value,"ie6.js")>0) {
                if($ie_version == "ie7") {
                    $ie_7_js = "<!--[if lt IE 7]>\r\n".$value."\r\n<![endif]-->";
                }
                $value = $ie_7_js;
            } else if (strpos($value,"html5shiv.js")>0) {
                if($ie_version == "ie9") {
                    $ie_9_js = "<!--[if lt IE 9]>\r\n".$value."\r\n<![endif]-->";
                }
                $value = $ie_9_js;
            } 
            if(strpos($value,"{'gtm.start':")>0 || strpos($value,"dataLayer = []")>0) {
                $gtm_js .= "\r\n".$value;
            } else {
                $js .= "\r\n".$value;
            }
        }
        preg_match_all('#(<noscript><iframe src="https://www.googletagmanager.com.*?</noscript>)#is', $page, $noscript_matches);
       
		$page = preg_replace('#<script.*?</script>#is', '', $page);
        $page = preg_replace('#</body>#',$js.'</body>',$page);

        $page = preg_replace('#(<noscript><iframe src="https://www.googletagmanager.com.*?</noscript>)#is', '', $page);
        $page = preg_replace('#</head>#',$gtm_js."\r\n".$noscript_matches[0][0].'</head>',$page);
