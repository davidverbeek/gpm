 ...
 */    
class Hs_Sales_Block_Linkpending extends Mage_Core_Block_Template
{
     public function addPendingOrderLink()
    {
        $parentBlock = $this->getParentBlock();
        
            $count = $this->getItemCount();
            if ($count == 1) {
                $text = $this->__('My current orders (%s)', $count);
            } elseif ($count > 0) {
                $text = $this->__('My current orders (%s)', $count);
            } else {
                $text = $this->__('My current orders');
            }
