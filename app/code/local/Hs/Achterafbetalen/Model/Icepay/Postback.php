 ...
 * 
 */
class Hs_Achterafbetalen_Model_Icepay_Postback {
    ...
    public function handle($_vars)
    {
       ...
       // $icepayTransaction = $this->sqlModel->loadPaymentByID($this->order->getRealOrderId());
        ...
        if ($this->canUpdateBasedOnIcepayTable($icepayTransaction['status'], $_vars['Status'])) {
            ...
            if ($_vars['Status'] == Icepay_IceCore_Model_Config::STATUS_SUCCESS || $_vars['Status'] == Icepay_IceCore_Model_Config::STATUS_OPEN)
        ...
        $_vars['Status'] = 'OPEN';
        Mage::getModel('achterafbetalen/icepay_result')->handle($_vars);
