 ...
 * @category    HS
 * @package     HS_Pickup
 * @copyright   Copyright (c) 2019 Helios Solution
 */
class Hs_Pickuplocation_Helper_Pickupaddress extends Mage_Core_Helper_Abstract
{
	/**
	...
	public function getPickupAddressFromQuote($quote)
	{
		...
		foreach ($quote->getAddressesCollection() as $_address) {
			if (!$_address->isDeleted() && $_address->getAddressType() == 'qls_pickup') {
				return $_address;
			}
		}
		return null;
	...
	public function setQuotePickupAddress($quote, $pickupAddress)
	{
		...
		if (!empty($currentPickupAddress)) {
			...
			$quote->addAddress($pickupAddress->setAddressType('qls_pickup'));
		}
	...
	public function savePickupAddressIntoQuote($quote, $pickupAddressData)
	{
		...
		$pickupAddress->setCompany($pickupAddressData['name'])
			->setPostcode($pickupAddressData['address']['postalcode'])
			->setCity($pickupAddressData['address']['locality'])
			->setCountryId($pickupAddressData['address']['country'])
			->setStreetFull($pickupAddressData['address']['street'] . "\n" . $pickupAddressData['address']['housenumber'])
			->setQlsServicepointCode($pickupAddressData['servicepoint_code']);
	...
	public function removePickupAddressFromQuote($quote)
	{
		/** @var Mage_Sales_Model_Quote_Address $_address */
		foreach ($quote->getAddressesCollection() as $_address) {
			if ($_address->getAddressType() == 'qls_pickup') {
