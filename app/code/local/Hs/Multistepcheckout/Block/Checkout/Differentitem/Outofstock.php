 ...
 */ 
class Hs_Multistepcheckout_Block_Checkout_Differentitem_Outofstock extends Mage_Sales_Block_Items_Abstract
{
    protected $_address;
    protected $_customer;
    protected $_quote;
    ...
    public function getItems()
    {
        ...
        /*foreach ($item as $_item){
            ...
            if(isset($result->VoorraadInfo) && !is_array($result->VoorraadInfo)) {
                ...
                $resultContent = $result->VoorraadInfo;
                $qty= $_item->getQty();    
                $truestock=$resultContent->Voorraad;
    ...
    public function getAddressesHtmlSelect($type)
    {
        ...
        foreach($stockData as $sku=>$stock){ 
            if(empty($stock->instock) || !isset($stock->instock)){
                $stockValue[]=$sku;
            }
        }
                ...
                foreach ($addressCollection as $_address)
                {
                    $addressCollectionItem=Mage::getModel('sales/quote_address_item')->getCollection()
                     ...
                     if($addressCollectionItem->getSize()> 0){ 
                        ...
                        if(!Mage::registry('outofstock_address'))
                        Mage::register('outofstock_address', $_address);
