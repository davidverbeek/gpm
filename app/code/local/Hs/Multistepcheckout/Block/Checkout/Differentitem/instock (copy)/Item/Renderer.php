    ...
    public function buildResult($result, $sku) {
        ...
        if($result->Voorraad <= 0) {
            if($result->Levertijd == 1) {
                $levertijd = '(Levertijd +/- ' . $result->Levertijd . ' dag)';
            } else {
                if($result->Levertijd > 14) {
                    $levertijd = '(Levertijd +/- 14 dagen)';
                } else {
                    $levertijd = '(Levertijd +/- ' . $result->Levertijd . ' dagen)';
                }
            }
            ...
            if($product->getData('leverancier')==3797){
                ...
                $newresult->text = '<span class="stock">geen voorraad, wel bestelbaar</span><span class="now-order">' . $levertijd . '</span>';
                ...
                if($diff->format("%R")==='+'){
                    $levertijd= $diff->format("%a")  + 2;
                }elseif($diff->format("%R")==='-'){
                    if($result->Levertijd == 0)
                        $levertijd= 5;
                    else
                        $levertijd= $result->Levertijd  + 2;
                }
