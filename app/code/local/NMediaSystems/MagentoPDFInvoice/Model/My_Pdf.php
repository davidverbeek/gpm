<?php
// from: http://framework.zend.com/manual/en/zend.pdf.pages.html
...
class My_Pdf extends Zend_Pdf {
    ...
    public static function drawText(Zend_Pdf_Page $page, $text, $x1, $y1, $x2 = null, $position = self::TEXT_ALIGN_LEFT, $encoding = null){
        ...
        $flags = array(
            "beginBold" => preg_match("/<b>/", $text),
            "endBold"   => preg_match("/<\/b>/", $text)
        );
        ...
        if ($flags["beginBold"]) {
			$page->setFillColor ( new Zend_Pdf_Color_Rgb(127,0,0) );
        }
    ...
    public static function drawTextBox(Zend_Pdf_Page $page, $text, $x1, $y1, $x2 = null, $position = self::TEXT_ALIGN_LEFT, $lineHeight = 1.1, $encoding = null){
        ...
        foreach( $lines as $line ){
            ...
            foreach( $words as $word ){
                $wordWidth = self::getTextWidth(strip_tags($word), $page);
    ...
    public static function createPages( Zend_Pdf_Page $template, $text, $margins=array(40,28,40,28), $position = self::TEXT_ALIGN_LEFT, $lineHeight = 1.1, $encoding = null ){
        ...
        $bottom = $y1;
        foreach( $lines as $line ){
            //$cFont = Zend_Pdf_Font::fontWithPath(Mage::getBaseDir().'/media/pdf/Source-Sans_Pro/SourceSansPro-Light.ttf');
            //$cFont = Zend_Pdf_Font::fontWithPath(Mage::getBaseUrl('media').'pdf/Source-Sans_Pro/SourceSansPro-Light.ttf');
            $cFont = Zend_Pdf_Font::fontWithPath('media/pdf/Source-Sans_Pro/SourceSansPro-Light.ttf');
    ...
    public static function getTextWidth($text, $resource, $fontSize = null, $encoding = null) {
        ...
        $drawingText = iconv ( $encoding, "ISO-8859-1//IGNORE", $text );
        $characters = array ();
