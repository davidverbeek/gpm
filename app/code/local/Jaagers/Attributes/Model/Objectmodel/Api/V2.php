...
class Jaagers_Attributes_Model_Objectmodel_Api_V2 extends Mage_Api_Model_Resource_Abstract
{
    ...
    public function createSet($name, $copyGroupsFromID = 4)
    {
        ...
        Mage::Log("Creating attribute-set with name " . $setName,null,'product_attribute.log');
        
        if($setName == '')
        {
        	Mage::Log("Could not create attribute set with an empty name.",null,'product_attribute.log');
        	return false;
        }
        ...
        Mage::Log("Using entity-type-ID" . $entityTypeID,null,'product_attribute.log');
        ...
        Mage::Log("Creating vanilla attribute-set with name [$setName].",null,'product_attribute.log');
        ...
        {
        	Mage::Log("Initial attribute-set with name [$setName] could not be saved: " . $ex->getMessage(),null,'product_attribute.log');
        	return false;
        }
        
        if(($id = $model->getId()) == false)
        {
        	Mage::Log("Could not get ID from new vanilla attribute-set with name [$setName].",null,'product_attribute.log');
        	return false;
        }
        
        Mage::Log("Set ($id) created.",null,'product_attribute.log');
        ...
        {
        	Mage::Log("Cloning group configuration from existing set with ID ($copyGroupsFromID).",null,'product_attribute.log');
        ...
        {
        	Mage::Log("Creating default group [{$this->groupName}] for set.",null,'product_attribute.log');
        ...
        {
        	Mage::Log("Final attribute-set with name [$setName] could not be saved: " . $ex->getMessage(),null,'product_attribute.log');
        	return false;
        }
        ...
        */
        ...
        Mage::Log("Created attribute-set with ID ($id) and default-group with ID ($copyGroupsFromID).",null,'product_attribute.log');
    ...
    function createAttribute($name, $attributeData, $store = null)
    {
        ...
        {
            Mage::Log("Can't import the attribute with an empty label or code.  LABEL= " . $labelText . " CODE= " . $attributeCode,null,'product_attribute.log');
            return false;
        }
        ...
        {
            Mage::Log("Attribute " . $labelText . " could not be saved: " . $ex->getMessage(),null,'product_attribute.log');
            return false;
        }
    ...
    public function editAttribute($name, $attributeData, $store = null) {
    	...
    	Mage::Log("Attribute has been edited".$name,null,'product_attribute.log');
    	...
    	if($labelText == '' || $attributeCode == '')
    	{
    		Mage::Log("Can't edit the attribute with an empty label or code.  LABEL= " . $labelText . " CODE= " . $attributeCode,null,'product_attribute.log');
    		return false;
    	}
    	...
    	Mage::Log($data,null,'product_attribute.log');
    	...
    	if ($attributeId) {
    		...
    		Mage::Log($attribute->getData(),null,'product_attribute.log');
	...
	public function addAttributeToSet($setId, $groupName, $attributeId)
	{
		...
		$attribute_group_id = $setup->getAttributeGroupId('catalog_product', $setId, $groupName);
		$return = false;
		try {
			$result = $setup->addAttributeToSet($entityTypeId = 'catalog_product', $setId, $attribute_group_id, $attributeId);
