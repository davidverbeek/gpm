...
class Queldorei_ShopperSettings_Block_Brands extends Mage_Core_Block_Template
{
	public function getBrands()
	{
		...
		$brandDir = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . 'catalog/brands/';
		$brandExt = Mage::getStoreConfig('shopperbrands/main/image', Mage::app()->getStore()->getId());
		$brands = array();
		$documentRoot = $_SERVER['DOCUMENT_ROOT'].DS.'media'.DS.'catalog'.DS.'brands';
		foreach ($brandsList as $b) {
			
			if(file_exists($documentRoot.DS.str_replace(' ', '_', strtolower($b)) . '.' . $brandExt)){
				$brands[] = array(
					'name' => $b,
