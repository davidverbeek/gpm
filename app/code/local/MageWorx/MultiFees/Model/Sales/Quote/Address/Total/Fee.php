...
class MageWorx_MultiFees_Model_Sales_Quote_Address_Total_Fee extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
	...
	/**
	 * This is the total data in the checkout - Techtwo
	 *
	 * @param Mage_Sales_Model_Quote_Address $address
	 * @return array|MageWorx_MultiFees_Model_Sales_Quote_Address_Total_Fee
	 */
	public function fetch(Mage_Sales_Model_Quote_Address $address)
    {
	    ...
	    $paymentMethodInstance = $checkout->getQuote()->getPayment() && $checkout->getQuote()->getPayment()->hasMethodInstance()? $checkout->getQuote()->getPayment()->getMethodInstance():NULL;
	    $method_title = $paymentMethodInstance? $paymentMethodInstance->getTitle():'';

        $helper = Mage::helper('multifees');
        if ($helper->isStoreMultifees() && $helper->isFeeEnabled()) {
            $address->addTotal(array(
                'code' => $this->getCode(),
	            // ($address->getTaxAmount() > 0 && $helper->isIncludingTax()) ? $helper->__('Additional Fees With Tax') : $helper->__('Additional Fees');
                'title' => 'Toeslag - '.$method_title,
