...
class MageWorx_MultiFees_Model_Observer {
    ...
    private function _prepareFeeOptions($fees, $method = null) {
        ...
        $exclShipping=$address->getShippingAmount();
        $subtotal=$subtotal+$exclShipping;
        ...
        return new Varien_Object(array('details' => $detailsFees, 'store' => $storeFee, 'final_price' => $resPrice,'price'=>$price));
    }
            ...
            if ($method) {
                ...
                if ($fees) {
                    try {
                        ...
                        $session->setStoreMultifees($options->getStore());
                        $session->setMultifees($options->getPrice());
                    } catch (Exception $e) {
                        $session->addException($e, $e->getMessage());
                    }
                }
