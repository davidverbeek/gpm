...
/*********************** STATUSES ***************************/
    ...
    public function getExpectedFormated()
    {
        date_default_timezone_set(Mage::getStoreConfig('general/locale/timezone'));
       //return substr(Mage::helper('core')->formatDate($this->getExpectedDeliveryDate(), $format, false),0,-4);
        $paymentCode = strtolower($this->getPayment()->getMethodInstance()->getCode());

        if($paymentCode == 'effectconnect_payment'){
            return 'De artikelen worden zo spoedig mogelijk verzonden';
        } else {
            return 'Deze artikelen verwachten wij te verzenden op <span style="font-weight: bold"> ' . Mage::helper('core')->formatDate($this->getExpectedDeliveryDate(), Mage_Core_Model_Locale::FORMAT_TYPE_FULL, false) . '</span>';
        }

        // return Mage::helper('core')->formatDate($this->getExpectedDeliveryDate(), Mage_Core_Model_Locale::FORMAT_TYPE_FULL, false);
    }
}
