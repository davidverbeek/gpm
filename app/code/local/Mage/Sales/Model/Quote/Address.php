...
class Mage_Sales_Model_Quote_Address extends Mage_Customer_Model_Address_Abstract
{
    ...
    // A. Standaard levering (default PostNL)
    const TRANSSMART_POSTNL_A_PROFILE_ID = '12789';
    const TRANSSMART_POSTNL_A_PROFILE_ID_DEV = '10437';
    // B. Handtekening ontvangst, alleen huis (signature and only homeaddress)
    const TRANSSMART_POSTNL_B_PROFILE_ID = '12790';
    const TRANSSMART_POSTNL_B_PROFILE_ID_DEV = '10438';
    // C. Verhoogd aansprakelijk (ensured)
    const TRANSSMART_POSTNL_C_PROFILE_ID = '12791';
    const TRANSSMART_POSTNL_C_PROFILE_ID_DEV = '10441';
    // D. Pak je gemak + Handtekening voor ontvangst (pickup at location via location selector)
    const TRANSSMART_POSTNL_D_PROFILE_ID = '12794';
    const TRANSSMART_POSTNL_D_PROFILE_ID_DEV = '10442';
    // Transmission
    const TRANSSMART_TRANSMISSION_PROFILE_ID = '19913';
    const TRANSSMART_TRANSMISSION_PROFILE_ID_DEV = '4652';
    // B2C
    const TRANSSMART_B2C_PROFILE_ID = '12788';
    const TRANSSMART_B2C_PROFILE_ID_DEV = '4643';
    ...
    public function getShippingRatesCollection()
    {
        if (is_null($this->_rates)) {
            ...
            // Pickup Location remove based on product attribute value "dhlservicepunt"
            $_isPickupNotAllow = $this->_isPickupNotAllow();
            foreach ($this->_rates as $key => $rate) {
                if (in_array($rate->getCarrier(), $_isPickupNotAllow)) {
    ...
    public function requestShippingRates(Mage_Sales_Model_Quote_Item_Abstract $item = null)
    {
        ...
        $SubtotalIncl=$this->getBaseSubtotal()+ $this->getBaseTaxAmount();
        $request->setPackageValue($item ? $item->getBaseRowTotal() : $SubtotalIncl);
        ...
        if ($result) {
            ...
            foreach ($shippingRates as $shippingRate) {
                ...
                // $transsmartCarrierProfileId = $rate->getData('transsmart_carrierprofile_id');

                // // SJD++ 28052018, Confirm, it's an effectconnect order
                // // for effectconnect order shipping rates should be zero
                // // don't allow pickup locations for effectconnect orders
                if (!empty($this->getQuote()->getData('effectconnect'))) {
                    $rate->setPrice(0.00);
                //     // PostNL D. Pak je gemak + Handtekening voor ontvangst (pickup at location via location selector)
                //     // Don't allow pickup location for effectconnect.
                //     if ($transsmartCarrierProfileId == Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_D_PROFILE_ID
                //         ||
                //         $transsmartCarrierProfileId == Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_D_PROFILE_ID_DEV) {
                //         continue;
                //     }
                } else {
                //     // SJD++ 26042018
                //     // Additional filter to collection as per transsmart shipping method selection.

                    // PP - for transmission product Shipping rate  
                    if($this->_isTransmissionTypePackage($rate)) {
                        if($rateTranssmission = $this->_getShippingCostExclBtw()){
                            $rate->setPrice($rateTranssmission);
                        } else {
                            continue;
                        }
                    }

                //     switch ($transsmartCarrierProfileId) {
                //         // PostNL A. Standaard levering (default PostNL)
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_A_PROFILE_ID:
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_A_PROFILE_ID_DEV:
                //             // PostNL B. Handtekening ontvangst, alleen huis (signature and only homeaddress)
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_B_PROFILE_ID:
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_B_PROFILE_ID_DEV:
                //             // PostNL C. Verhoogd aansprakelijk (ensured)
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_C_PROFILE_ID:
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_C_PROFILE_ID_DEV:
                //             // PostNL D. Pak je gemak + Handtekening voor ontvangst (pickup at location via location selector)
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_D_PROFILE_ID:
                //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_D_PROFILE_ID_DEV:
                //             if ($this->getSubtotalInclTax() > 100) {
                //                 $rate->setPrice(0.00);
                //             }
                //             break;
                //         default:
    ...
    /**
     * SJD++ 26042018
     * Validate transsmart shipping methods if they are available or not
     *
     * @param $quoteAddressRate
     * @return bool
     */
    // private function _shouldApplyQuoteAddressRate($quoteAddressRate)
    // {
    //     $transsmartCarrierProfileId = $quoteAddressRate->getData('transsmart_carrierprofile_id');
    //     $isTransmissionPackage = $this->_isTransmissionTypePackage();

    //     switch ($transsmartCarrierProfileId) {
    //         // PostNL A. Standaard levering (default PostNL)
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_A_PROFILE_ID:
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_A_PROFILE_ID_DEV:
    //             if (!$isTransmissionPackage && $this->getSubtotalInclTax() < 100
    //                 &&
    //                 $quoteAddressRate->getCarrier() != 'tablerate') {
    //                 return true;
    //             }
    //             break;
    ...
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_B_PROFILE_ID:
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_POSTNL_B_PROFILE_ID_DEV:
    //             if (!$isTransmissionPackage
    //                 &&
    //                 ($this->getSubtotalInclTax() >= 100 && $this->getSubtotalInclTax() < 500)
    //                 &&
    //                 $quoteAddressRate->getCarrier() != 'tablerate') {
    //                 return true;
    //             }
    ...
    //         // Transmission
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_TRANSMISSION_PROFILE_ID:
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_TRANSMISSION_PROFILE_ID_DEV:
    //             if ($isTransmissionPackage
    //                 &&
    //                 $quoteAddressRate->getCarrier() == 'tablerate') {
    //                 return true;
    ...
    //         // B2C
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_B2C_PROFILE_ID:
    //         case Mage_Sales_Model_Quote_Address::TRANSSMART_B2C_PROFILE_ID_DEV:
    //             if (!$isTransmissionPackage
    //                 &&
    //                 $quoteAddressRate->getCarrier() == 'tablerate') {
    //                 return true;
    ...
    //         default: // Non transsmart shipping methods not allowed
    //             return false;
    //             break;
    //     }
    ...
    /**
     * SJD++ 24042018
     * Finds if the quote is of transmission type shipment or not.
     *
     * @param Mage_Sales_Model_Quote $_quote
     * @return bool
     */
    private function _isTransmissionTypePackage()
    {
        $items = $this->getAllVisibleItems();
        foreach ($items as $_item) {
            $_resource = Mage::getSingleton('catalog/product')->getResource();
            $transmissionValue = $_resource->getAttributeRawValue(
                $_item->getProductId(), 'tansmission', Mage::app()->getStore()->getId()
            );
            // If any one product has transmission value it's Transmission shipment package
    ...
    /**
     * PP++ 17052018
     * Finds if the quote is of Pickup Location not allow.
     *
     * @param Mage_Sales_Model_Quote $_quote
     * @return bool
     */
    private function _isPickupNotAllow()
    {
        $items = $this->getAllVisibleItems();
        foreach ($items as $_item) {
            $_resource = Mage::getSingleton('catalog/product')->getResource();
            $pickupNotAllow = $_resource->getAttributeRawValue(
                $_item->getProductId(), 'dhlservicepunt', Mage::app()->getStore()->getId()
            );
            // If any one product has disabled for pickup
            if($pickupNotAllow) {
                return array('hs_pickuplocation');
            }
        }
    ...
    /**
     * PP++ 10042019
     * Return Transmmission product shipping cost based on Subtotal and country selection
     *
     * @param Mage_Sales_Model_Quote $_quote
     * @return bool
     */
    private function _getShippingCostExclBtw() {
        $shippingCost = array(
            'NL' => array(
                '11.9835',
                '8.6778'
        ...
        if(isset($shippingCost[$this->getCountry()])){
            return ($this->getSubtotalInclTax()>=100)?$shippingCost[$this->getCountry()][1]:$shippingCost[$this->getCountry()][0];
        }
