    ...
    { //echo get_class($item)."==";
        ...
        if ($quoteItem->getParentItem() && $quoteItem->isChildrenCalculated()) {
            ...
            if($quoteItem->getCustomPrice()){
                $finalPrice = $quoteItem->getCustomPrice();
                $item->setPrice($finalPrice)
                    ->setBaseOriginalPrice($finalPrice);
            }
            else{
            ...
            if ($routeName == 'multistepcheckout' && $action == 'updatecartItem') {  
                if ($item instanceof Mage_Sales_Model_Quote_Address_Item) { 
                    ...
                    foreach($quotesItems as $_quoteitem) {
                      if($_quoteitem->getId()==$item->getQuoteItemId())  {
                          $quoteItemUpdate=$_quoteitem;
                          
                         $quoteItemUpdate->calcRowTotal();
                        $this->_addAmount($quoteItemUpdate->getRowTotal());
                        $this->_addBaseAmount($quoteItemUpdate->getBaseRowTotal());
                        $address->setTotalQty($address->getTotalQty() + $quoteItemUpdate->getQty()); 
                      }
                    }
                    /*$quoteItemUpdate=Mage::getModel('sales/quote_item')->getCollection()
