...
class Mage_Tax_Model_Sales_Total_Quote_Subtotal extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    ...
    { 
        ...
        if ($routeName == 'multistepcheckout' && $action == 'updatecartItem') {  
                if ($item instanceof Mage_Sales_Model_Quote_Address_Item) { 
                    $quotesItems=Mage::getSingleton('checkout/session')->getQuote()->getAllItems();
                    foreach($quotesItems as $_quoteitem) { //echo $item->getQuoteItemId(); exit;
                      if($_quoteitem->getId()==$item->getQuoteItemId())  {
                          $quoteItemUpdate=$_quoteitem;
                          
                            $quoteItemUpdate->setPrice($basePrice);
                            $quoteItemUpdate->setBasePrice($basePrice);
                            $quoteItemUpdate->setRowTotal($subtotal);
                            $quoteItemUpdate->setBaseRowTotal($baseSubtotal);
                            $quoteItemUpdate->setPriceInclTax($taxPrice);
                            $quoteItemUpdate->setBasePriceInclTax($baseTaxPrice);
                            $quoteItemUpdate->setRowTotalInclTax($taxSubtotal);
                            $quoteItemUpdate->setBaseRowTotalInclTax($baseTaxSubtotal);
                            $quoteItemUpdate->setTaxableAmount($taxable);
                            $quoteItemUpdate->setBaseTaxableAmount($baseTaxable);
                            $quoteItemUpdate->setIsPriceInclTax($isPriceInclTax);
                            if ($this->_config->discountTax($this->_store)) {
                                $quoteItemUpdate->setDiscountCalculationPrice($taxPrice);
                                $quoteItemUpdate->setBaseDiscountCalculationPrice($baseTaxPrice);
                            }
                            $quoteItemUpdate->save();
                      }
                    }
                }
        }
        
       // echo get_class($item).$basePrice."==".$subtotal."===".$taxSubtotal; exit;
