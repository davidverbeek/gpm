...
class Mage_Catalog_Model_Product_Api_V2 extends Mage_Catalog_Model_Product_Api
{
    ...
    {
		Mage::log(print_r($productData,true),null,'prod.log');
		Mage::log("product ID :".$productId,null,'updatePInfo.log');
        $product = $this->_getProduct($productId, $store, $identifierType);

		$productData=$this->metaTagGenerator($productId,$productData);
        ...
        if (property_exists($productData, 'additional_attributes')) {
            if (property_exists($productData->additional_attributes, 'single_data')) {
                foreach ($productData->additional_attributes->single_data as $_attribute) {
                     ...
                     * Date:16-oct-2014
                     * defination: Fetch the attribute optoin id from value
                    */
					$productModel = Mage::getSingleton('catalog/product');
					$attr = $productModel->getResource()->getAttribute($_attribute->key);
					if ($attr->usesSource()) {
						$_attribute->value=$attr->getSource()->getOptionId($_attribute->value);
					}
    ...
    /**
     * Retrieve product Meta Tag generator
     * @param int|string $productId
     * @param string|int $store
     * @return array
     */
    private function metaTagGenerator($productId,$productData)
	{
        $model=Mage::getModel('catalog/product')->loadByAttribute('sku',$productId);
        $percentKeywords = Mage::getStoreConfig('metataggenerator/general/max');
        $enableSuggestions = Mage::getStoreConfig('metataggenerator/general/enable_suggestion');
        $enableModule = Mage::getStoreConfig('metataggenerator/general/enable');
        $metaModel = Mage::getModel('metataggenerator/MetaTagGenerator');
        $totalKeywords = $percentKeywords / 100;
        $title = $model->getName();
        $metaInfo="";
        if($enableModule) {
            
            $metaInfo = $metaModel->getMetaInfo($title, $model->getDescription(), $totalKeywords);			
            $productData->meta_title=$title;
            $productData->meta_keyword=$metaInfo[0];
            $productData->meta_description=$metaInfo[1];			
        }
        
        Mage::log(print_r($productId,true),null,'prod_metatag.log');
