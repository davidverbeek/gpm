...
// added more attribute which will be used in the vertical navigation
...
class Mage_Catalog_Model_Observer
{
    ...
    protected function _addCategoriesToMenu($categories, $parentCategoryNode, $menuBlock, $addTags = false)
    {
        ...
        foreach ($categories as $category) {
			...
			$productcnt = $this->getCategoryCount($category); 
			if (!$productcnt) {
                continue;
            }
            if ($addTags) {
            ...
            $categoryData = array(
                ...
                'is_topcategory' => $category->getData('is_topcategory'),
                'category_short_headline' => $category->getData('category_short_headline'),
                'category_level3_banner' => $category->getData('category_level3_banner')
            );
	...
	/**
     * Checks whether category has any products
     *
     */
	public function getCategoryCount($category)
    {
		$cat = Mage::getModel('catalog/category')->load($category->getId());
		return $cat->getProductCount();
