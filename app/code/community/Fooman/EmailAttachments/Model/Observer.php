...
class Fooman_EmailAttachments_Model_Observer
{
	...
	public function beforeSendOrder($observer)
	{
		...
		if (!$order->hasData()) {
			Mage::log($mailTemplate->getEntityId(),null,"email_queue_order_not_found.log");
		}
		...
		if (Mage::getStoreConfig('sales_email/' . $configPath . '/attachpdf', $storeId)  && $paymentCode != 'achterafbetalen' ) {

			// Order Confirmation PDF - commented as client request to attach invoice PDF instead confirmation PDF. - PP25052018
			...
			try {
				...
				Mage::log($e->getMessage(),null,'Invoice - Generate Error');
			}
			...
			$nonInvoicebleMethods = array('achterafbetalen');
    	// Removed Effect Connect on 23rd Nov by Parth

			if (count($invoicesSet) > 0 && !in_array($paymentCode,$nonInvoicebleMethods)) {

				$pdf = Mage::getModel('sales/order_pdf_invoice')->getPdf(array($order->getInvoiceCollection()->getFirstItem()));
				$mailTemplate = Mage::helper('emailattachments')->addAttachment($pdf,$mailTemplate,Mage::helper('sales')->__('Invoice')."_".$order->getIncrementId());
			}
		}
	...
	public function beforeSendShipment($observer)
	{
		...
		$paymentCode = strtolower($shipment->getOrder()->getPayment()->getMethodInstance()->getCode());
    $nonInvoicebleMethods = array('achterafbetalen');
    // Removed Effect Connect on 23rd Nov by Parth

		if (Mage::getStoreConfig('sales_email/' . $configPath . '/attachpdf', $storeId) && !in_array($paymentCode,$nonInvoicebleMethods)) {
			...
			Mage::helper('emailattachments')->addAttachment(
				$pdf,
				$mailTemplate,
				Mage::helper('sales')->__('Invoice') . "_" . $shipment->getOrder()->getIncrementId()
			);
	...
	protected function _processSendQueueEvent($mailer, $message)
	{
		if ($message->getEntityType() == 'order' && !$message->getProcessedAt()) {
			...
			if (!$order->hasData()) {
				Mage::log($message->getEntityId(),null,"email_queue_order_not_found.log");
			}
    		...
    		// Removed Effect Connect on 23rd Nov by Parth
				...
				if (count($invoicesSet) > 0 && !in_array($paymentCode,$nonInvoicebleMethods)) {
					...
					$pdf = Mage::getModel('sales/order_pdf_invoice')->getPdf(array($order->getInvoiceCollection()->getFirstItem()));
					Mage::helper('emailattachments')->addAttachment($pdf,$mailer,Mage::helper('sales')->__('Invoice')."_".$order->getIncrementId());
				}
			}
