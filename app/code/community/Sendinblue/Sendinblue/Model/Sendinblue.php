...
class Sendinblue_Sendinblue_Model_Sendinblue extends Mage_Core_Model_Abstract
{
    ...
    */
    ...
    public function allAttributesName()
    {
        if ($this->userLanguage == 'fr') {
            $attributesName = array('PRENOM'=>'firstname', 'NOM'=>'lastname', 'MAGENTO_LANG'=>'created_in','CLIENT'=>'client','SMS'=>'telephone','COMPANY'=>'company','CITY'=>'city','COUNTRY'=>'country_id','POSTCODE'=>'postcode','STREET'=>'street','REGION'=>'region','STORE_ID'=>'store_id');
        }
        else {
            $attributesName = array('NAME'=>'firstname', 'SURNAME'=>'lastname', 'MAGENTO_LANG'=>'created_in','CLIENT'=>'client','SMS'=>'telephone','COMPANY'=>'company','CITY'=>'city','COUNTRY'=>'country_id','POSTCODE'=>'postcode','STREET'=>'street','REGION'=>'region','STORE_ID'=>'store_id','KLANTGROEP'=>'debternumber','KORTINGSGROEP'=>'store_id');
        }
    ...
    */
    public function getcustomers()
    {
        ...
        ->addAttributeToSelect(array('email', 'firstname', 'lastname', 'group_id', 'created_in'))
		...
		->joinField('debternumber','mage_customer_entity_varchar','value','entity_id=entity_id',"{{table}}.attribute_id='134'");
        ...
        foreach ($subscribersCollection->getData() as $subscriber) {
			...
			/*$customerObject = Mage::getModel('customer/customer')->setWebsiteId(1)->loadByEmail($email);
			if ($customerObject->getId()) {  
			
			
				$debternumber = $customerObject['debternumber'];
				$groupId = $customerObject['group_id'];
				$group_name = Mage::getModel('customer/group')->load($groupId)->getCustomerGroupCode();		
			...
			FROM
				mage_customer_group cg
			WHERE
				c.group_id = cg.customer_group_id) AS customer_group,
			(SELECT 
				fn.value
			FROM
				mage_customer_entity_varchar fn
			WHERE
				c.entity_id = fn.entity_id
					AND fn.attribute_id = 134) AS debternumber
			FROM
			 mage_customer_entity AS c WHERE c.email = '$email'
			GROUP BY entity_id";

			$customer_data = $readConnection->fetchAll($query);
			

			foreach ($customer_data as $custvalue) {
				$debternumber = $custvalue['debternumber'];
				$group_name = $custvalue['customer_group'];
			}
			...
			//Non-registered users i.e users only subscribed for newsletter
                ...
                foreach ($attributes as $alias => $attribute) {
					...
					$contact['KLANTGROEP'] = $group_name;
					$contact['KORTINGSGROEP'] = ($debternumber != '') ? $debternumber : '400005';
					
                }
            }
			//echo 'debternumber'.$debternumber;
    ...
    */
    public function getregisteredcustomers()
    {
        ...
        foreach ($customersCollection as $customerCollection) {
            ...
            $email = $customer['email'];
            $customers[$email]['EMAIL'] = $email;
			$debternumber = $customer['debternumber'];
            $groupId = $customer['group_id'];
			
			$query = "SELECT customer_group_code FROM mage_customer_group WHERE customer_group_id ='$groupId'";
			$customer_data = $readConnection->fetchAll($query);
			

			foreach ($customer_data as $custvalue) {
			
				$group_name = $custvalue['customer_group_code'];
			}
			
            foreach ($attributes as $alias => $attribute) {
                $customers[$email][$alias] = isset($customer[$attribute]) ? $customer[$attribute] : '';
				$customers[$email]['CLIENT'] = 0;
                $customers[$email]['MAGENTO_LANG'] = 'Admin';
				$customers[$email]['KORTINGSGROEP'] = ($debternumber != '') ? $debternumber : '400005';
				$customers[$email]['KLANTGROEP'] = $group_name;
            }
            $customers[$email]['CLIENT'] = $customer['entity_id'] > 0 ? 1 : 0;
            if (!empty($customer['country_id']) && !empty($customer['telephone'])) {
                $customers[$email]['SMS'] = $this->checkMobileNumber($customer['telephone'], $this->getCountryCode($customer['country_id']));
     ...
     * them in the Sendinblue magento module.
    */
    public function gettransactionalcustomers()
    {
		...
		$strSkuQuery = "
			SELECT 
				msfo.customer_email AS email,
				msfo.customer_id AS customer_id,
				billing.telephone AS telephone,
				billing.company AS company,
				billing.city AS city,
				billing.postcode AS postcode,
				billing.region AS region,
				billing.country_id AS country_id,
				billing.firstname AS firstname,
				billing.lastname AS lastname,
				COUNT(*) AS order_count				
			FROM
				`mage_sales_flat_order` AS msfo
				JOIN `mage_sales_flat_order_address` AS billing
				ON billing.parent_id = msfo.entity_id
			WHERE
				msfo.customer_email != '' AND billing.address_type = 'billing'
			GROUP BY msfo.customer_email
			HAVING order_count >= 1";
		$customersCollection = $readConnection->fetchAll($strSkuQuery);
		unset($resource);
		unset($readConnection);
