...
Transsmart.Shipping.Pickup = Class.create({
    ...
    initialize: function(config) {
        ...
        // SJD++ 16052018, START transsmart customization.
        // Display appropiate delivery address (either of pickup location or shipping address) on shipping method selection
        jQuery(document).on('click', 'input[name=shipping_method]', function (e) {
            jQuery('.default-deliveryaddress > .shipping-row > .pickup-location-address-display').addClass('hidden');
            jQuery('.default-deliveryaddress > .shipping-row > .form-list').removeClass('hidden');

            var currentMethod = jQuery(this);
            if(currentMethod.prop('checked') &&
                currentMethod.val().indexOf('transsmartpickup_carrierprofile') >= 0) {
                jQuery('.default-deliveryaddress > .shipping-row > .pickup-location-address-display').removeClass('hidden');
                jQuery('.default-deliveryaddress > .shipping-row > .form-list').addClass('hidden');
            }
        });
        // SJD++ 16052018, END transsmart customization.
    },
    ...
    selectLocationAndClose: function() {
        if (this.selectedMarker != null && this.selectedMarker.locationData) {
            ...
            // SJD++ 16052018, START transsmart customization.
            // pickup location address replace with delivery address when selected.
            var strPickupAddress = '<span class="pickup-location-address-display">' +
                locationData.street.escapeHTML() + ' ' +
                locationData.street_no.escapeHTML() + ', ' +
                locationData.zipcode.escapeHTML() + ', ' +
                locationData.city.escapeHTML();
            if(territoryLocaleList != 'undefined') {
                strPickupAddress += ', ' + territoryLocaleList[locationData.country];
            }
            strPickupAddress += '</span>';
            jQuery('.default-deliveryaddress > .shipping-row > .pickup-location-address-display').remove();
            jQuery('.default-deliveryaddress > .shipping-row > .form-list').addClass('hidden').after(strPickupAddress);
            // SJD++ 16052018, END transsmart customization.
